...
void build(int node, int start, int end,int p)
{
    if(start == end)
    {
    ...
    else
    {
        ...
        build(2*node, start, mid,p-1);
        // Recurse on the right child
        build(2*node+1, mid+1, end,p-1);
...
void update(int node, int start, int end, int idx, int val,int p)
{
    ...
    else
    {
        ...
        if(start <= idx and idx <= mid)
        {
            ...
            update(2*node, start, mid, idx, val,p-1);
        }
        else
        {
            // if idx is in the right child, recurse on the right child
            update(2*node+1, mid+1, end, idx, val,p-1);
        }
        // Internal node will have the sum of both of its children
        if(p%2==1)
        tree[node] = tree[2*node] | tree[2*node+1];
